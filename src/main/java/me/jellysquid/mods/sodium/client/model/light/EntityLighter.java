...
public class EntityLighter {
    ...
    public static <T extends Entity> int getBlendedLight(T entity, float tickDelta) {
        ...
        for (int bX = bMinX; bX < bMaxX; bX++) {
            ...
            for (int bY = bMinY; bY < bMaxY; bY++) {
                ...
                for (int bZ = bMinZ; bZ < bMaxZ; bZ++) {
                    ...
                    if (calcBlockLight) {
                        bl += weight * ((entity.isOnFire() ? 15 : entity.world.getLightLevel(LightType.BLOCK, pos)) / MAX_LIGHT_VAL);
                    } else {
                        bl += weight;
                    }
                }
